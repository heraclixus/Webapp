...
#     list_dat = []
#     for list in links:
#         if list.submitter_id == 62443:
#             list_dat.append(list)
#     # list_dat.sort(key = lambda list: list[2])
#     list_dat.sort(key=lambda x: x.submitted_time)
#     return list_dat
...
# def query():
#     c = db.execute("select * from links where submitted_id = 62443 and votes > 1000")
#     link = Link(*c.fetchone())
#     return link.id

def query():
    c = db.execute("select id from links where submitted_id =  62443 and order by submitted_time asc ")
